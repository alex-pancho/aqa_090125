...
car_data_filtered = []
car_data_filtered_sorted_joined = []

car_data_to_set_values = set(car_data.values())

for value in car_data_to_set_values:
    if value[1] >= search_criteria[0]:
        if value[2] >= search_criteria[1]:
            if value[-1] <= search_criteria[-1]:  
                car_data_filtered.append(value)

car_data_filtered_sorted = sorted(car_data_filtered, key=lambda tup: tup[-1])

dict_keys = []
dict_val = []

for val in car_data_filtered_sorted:
